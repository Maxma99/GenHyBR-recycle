...
thetaqr = [1.0 1.0];      %For now set them as isotropic

% Build row/column of the Toeplitz matrix
Qr = createrow(xmin,xmax,nvec,k,thetaqr);
...
[x_0, ~, trunc_mats] = genHyBRrecycle(A,bn(:),P,Q,R,input,trunc_options, trunc_mats);
x_0 = reshape(x_0,n,n);
W = trunc_mats.W;
...
tic
[xthybr_opt, toutput_opt, trunc_mats] = genHyBRrecycle(A2, bn2(:),P,Q,R,input, trunc_options, trunc_mats);
t1 = toc;
