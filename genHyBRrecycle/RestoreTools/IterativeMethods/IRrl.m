function [x, IterInfo] = IRrl(A, b, options, sigmaSq, beta)
%
%      [x, IterInfo] = IRrl(A, b, options;
%
...
if ~noIterBar
  h = waitbar(0, 'Beginning IRrl iterations: please wait ...');
end
...
for k = 1:MaxIter
  ...
  c = A*x + beta*ones(sx) + sigmaSq*ones(size(x));
  trAb = A'*b;
