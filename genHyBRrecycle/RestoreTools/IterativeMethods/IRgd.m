function [x, IterInfo] = IRgd(A, b, options)
%
%      [x, IterInfo] = IRgd(A, b, options);
%
%  Gradient Descent Iterative Method for Image Restoration.  This can be
%  used to solve least squares problem 
%     min ||b - Ax||
%
...
% 
%  If A is full or sparese then shape b as a vector
%
...
if ~noIterBar
  if StepLength == 0
    h = waitbar(0, 'Beginning IRgd (steepest descent) iterations: please wait ...');
  else
    h = waitbar(0, 'Beginning IRgd (Richardson) iterations: please wait ...');
  end 
end
